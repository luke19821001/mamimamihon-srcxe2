...
procedure zengyuan(mods, id, zyid: integer);
...
//增加人物到增援队列
procedure addzengyuanarr(TmpZYdui:TTmpZYdui; id, mpnum, snum:integer);
function is_hufa(rnum:integer):boolean;
...
begin
  ...
  mpbdata[id].key := -1;
  if ((mods = -2) and (bstatus = 1)) or ((mods = -3) and (bstatus = 2)) then //防御获胜或者进攻失败
  begin
    ...
    begin
      ...
      Rrole[mpbdata[id].BTeam[1].RoleArr[j].rnum].lsweizhi := -1;
      Rrole[mpbdata[id].BTeam[1].RoleArr[j].rnum].nweizhi := -1;
      Rrole[mpbdata[id].BTeam[1].RoleArr[j].rnum].btnum:=-1;
      if mods = -2 then
      begin
    ...
    begin
      Rrole[mpbdata[id].BTeam[0].RoleArr[j].rnum].btnum:=-1;
      Rrole[mpbdata[id].BTeam[0].RoleArr[j].rnum].weizhi := mpbdata[id].BTeam[0].RoleArr[j].snum;
      Rrole[mpbdata[id].BTeam[0].RoleArr[j].rnum].lsweizhi := -1;
      if mods = -3 then
    ...
    for j :=0 to  mpbdata[id].ZYlen -1 do
    begin
      Rrole[mpbdata[id].zengyuan[j].duizhang.rnum].lsweizhi:= -1;
      Rrole[mpbdata[id].zengyuan[j].duizhang.rnum].weizhi:=mpbdata[id].zengyuan[j].duizhang.snum;
      Rrole[mpbdata[id].zengyuan[j].duizhang.rnum].btnum := -1;
      Rrole[mpbdata[id].zengyuan[j].duizhang.rnum].dtime := 5;
      Rrole[mpbdata[id].zengyuan[j].duizhang.rnum].nweizhi := 14;
      for j1 := 0 to length(mpbdata[id].zengyuan[j].duiyuan) - 1 do
      begin
        Rrole[mpbdata[id].zengyuan[j].duiyuan[j1].rnum].lsweizhi:= -1;
        Rrole[mpbdata[id].zengyuan[j].duiyuan[j1].rnum].weizhi:=mpbdata[id].zengyuan[j].duiyuan[j1].snum;
        Rrole[mpbdata[id].zengyuan[j].duiyuan[j1].rnum].btnum := -1;
        Rrole[mpbdata[id].zengyuan[j].duiyuan[j1].rnum].dtime := 5;
        Rrole[mpbdata[id].zengyuan[j].duiyuan[j1].rnum].nweizhi := 14;
      end;
    end;
  end
  ...
  begin
    ...
    begin
      Rrole[mpbdata[id].zengyuan[j].duizhang.rnum].lsweizhi:= -1;
      if Rrole[mpbdata[id].zengyuan[j].duizhang.rnum].menpai >= 0 then
      begin
        Rrole[mpbdata[id].zengyuan[j].duizhang.rnum].weizhi:=mpbdata[id].zengyuan[j].duizhang.snum;
      end;
      Rrole[mpbdata[id].zengyuan[j].duizhang.rnum].btnum := -1;
      ...
      begin
        Rrole[mpbdata[id].zengyuan[j].duiyuan[j1].rnum].lsweizhi:= -1;
        if Rrole[mpbdata[id].zengyuan[j].duiyuan[j1].rnum].menpai >= 0 then
        begin
          Rrole[mpbdata[id].zengyuan[j].duiyuan[j1].rnum].weizhi:=mpbdata[id].zengyuan[j].duiyuan[j1].snum;
        end;
        Rrole[mpbdata[id].zengyuan[j].duiyuan[j1].rnum].btnum := -1;
...
begin
  Tmpzydui1.len:=0;
  Tmpzydui2.len:=0;
  Tmpzydui1.duizhang := -1;
  Tmpzydui2.duizhang := -1;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            if (Tmpzydui1.duizhang < 0) and ((Rmenpai[Rrole[i].menpai].zmr = i) or (is_hufa(i))) then
            begin
    ...
    begin
      addzengyuanarr(Mytmpzydui[i],id,Rscene[mpbdata[id].snum].menpai,mpbdata[id].snum);
    end;
    if len > 0 then levsort(trnum);
...
procedure addzengyuanarr(TmpZYdui:TTmpZYdui; id , mpnum,snum:integer);
var
  len1, len2, len3, t, t0, i:integer;
begin
  t:=timetonum;
  len3:=0;
  if (TmpZYdui.duizhang > 0) then
  begin
    if (Rrole[TmpZYdui.duizhang].nweizhi = 16) and (Rrole[TmpZYdui.duizhang].weizhi = snum) then
      exit;
    if Rrole[TmpZYdui.duizhang].nweizhi <> 16 then
    begin
      t0 := t + Rrole[TmpZYdui.duizhang].dtime;
    end
    else
    begin   //防守场景有人员正在其他战场作战时，其撤离战场
      t0 := t + 5 +random(5);
      len1:=length(mpbdata[Rrole[TmpZYdui.duizhang].btnum].zaotui);
      if len1 <= 0 then
      begin
        len1 := 0;
      end;
      setlength(mpbdata[Rrole[TmpZYdui.duizhang].btnum].zaotui,len1 + 1);
      mpbdata[Rrole[TmpZYdui.duizhang].btnum].zaotui[len1].rnum := TmpZYdui.duizhang;
      mpbdata[Rrole[TmpZYdui.duizhang].btnum].zaotui[len1].snum := Rrole[TmpZYdui.duizhang].weizhi;
      mpbdata[Rrole[TmpZYdui.duizhang].btnum].zaotui[len1].btnum := id;
    end;
    if ismengyou(Rrole[TmpZYdui.duizhang].menpai,mpnum) then
    begin

      t0 :=t + Rrole[TmpZYdui.duizhang].dtime + 10 * 1000 div Rmenpai[Rrole[TmpZYdui.duizhang].menpai].guanxi[mpnum];
    end;
    len2:=mpbdata[id].ZYlen;
    setlength(mpbdata[id].zengyuan, len2 + 1);
    inc(mpbdata[id].ZYlen);
    mpbdata[id].zengyuan[len2].duizhang.rnum := TmpZYdui.duizhang;
    if (Rrole[TmpZYdui.duizhang].nweizhi <> 16) then
    begin
      mpbdata[id].zengyuan[len2].duizhang.snum := Rrole[TmpZYdui.duizhang].weizhi;
    end;
    mpbdata[id].zengyuan[len2].duizhang.isin := 0;
    aotosetmagic(TmpZYdui.duizhang);
    mpbdata[id].zengyuan[len2].duizhang.mag := GetGeliveablemag(TmpZYdui.duizhang);
    mpbdata[id].zengyuan[len2].duizhang.day := t0;
    Rrole[TmpZYdui.duizhang].lsweizhi := snum;
    Rrole[TmpZYdui.duizhang].nweizhi := 16;
    Rrole[TmpZYdui.duizhang].dtime := 1000;
    Rrole[TmpZYdui.duizhang].btnum := id;
    for i:=0 to TmpZYdui.len - 1 do
    begin
      if (Rrole[TmpZYdui.duizhang].nweizhi = 16) and (Rrole[TmpZYdui.duizhang].weizhi = snum) then
        continue;
      if (TmpZYdui.duiyuan[i] <> 0) and ((Rrole[TmpZYdui.duiyuan[i]].nweizhi = 16) or (Rrole[TmpZYdui.duiyuan[i]].dtime <= t0)) then
      begin
        setlength(mpbdata[id].zengyuan[len2].duiyuan, len3 + 1);
        mpbdata[id].zengyuan[len2].duiyuan[len3].rnum := TmpZYdui.duiyuan[i];
        if (Rrole[TmpZYdui.duiyuan[i]].nweizhi <> 16) then
        begin
          mpbdata[id].zengyuan[len2].duiyuan[len3].snum := Rrole[TmpZYdui.duiyuan[i]].weizhi;
        end;
        mpbdata[id].zengyuan[len2].duiyuan[len3].isin := 0;
        aotosetmagic(TmpZYdui.duiyuan[i]);
        mpbdata[id].zengyuan[len2].duiyuan[len3].mag := GetGeliveablemag(TmpZYdui.duiyuan[i]);
        mpbdata[id].zengyuan[len2].duiyuan[len3].day := t0;
        Rrole[TmpZYdui.duiyuan[i]].lsweizhi := snum;
        Rrole[TmpZYdui.duiyuan[i]].nweizhi := 16;
        Rrole[TmpZYdui.duiyuan[i]].dtime := 1000;
        Rrole[TmpZYdui.duiyuan[i]].btnum := id;
        inc(len3);
      end;
    end;
  end;
...
begin
  ...
  begin
    ...
    begin
      Brole[i].frozen := 0;
      p:=(power(Brole[i].speed / 7, 0.55) +
       power(max(1, Rrole[Brole[i].rnum].CurrentMP - 200) / 40, 0.5) * (100 + Brole[i].zhuangtai[13]) *
        (Brole[i].zhuangtai[3] + 100) * (100 + Rrole[Brole[i].rnum].PhyPower) * (100 + Rrole[Brole[i].rnum].Angry) / 400000000);
      Inc(Brole[i].Progress, trunc(p));
...
procedure BattleMainControl(mods, id: integer); overload;
var
  i, j, j1, j2, j3, jj, n, i1, i2, add, a, k, mpnum0, mpnum1,tmp,zyid: integer;
...
begin
  ...
  begin
    ...
    begin
        ...
        writeln(debugfile,'战斗人物'+inttostr(i));
        flush(debugfile);
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            Rrole[zy0[j1][0]].lsweizhi := mpbdata[id].snum;
            ...
            begin
              ...
              begin
                ...
                mpbdata[id].BTeam[0].rolearr[jj].isin := 0;
                Rrole[j2].lsweizhi := mpbdata[id].snum;
                Rrole[j2].nweizhi := 16;
                Rrole[j2].dtime := 1000;
                Inc(jj);
              end;
          ...
          begin
            ...
            Rrole[zy1[j1][0]].lsweizhi := mpbdata[id].snum;
      ...
      begin
        ...
        if (jiqicount > 1500) and (event.key.keysym.sym = SDLK_Q) then
        begin
          setlength(menuString2,3);
          menuString2[0]:='是否逃離戰鬥（認輸）？';
          menuString2[1]:='認輸';
          menuString2[2]:='戰鬥';
...
begin
  ...
  DrawShadowText(@str[1], screen.w - 65,37, ColColor($5), ColColor($7));
  if jiqicount > 1500 then
  begin
    str := '戰鬥持續太久，可按Q鍵逃離';
    DrawShadowText(@str[1], 100,5, ColColor($5), ColColor($7));
  end;
end;
...
begin
  ...
  twice := 0;
  r1 := 0;
  if (GetEquipState(rnum, 26) or (GetGongtiState(rnum, 26))) then
  begin
    for i1 := 0 to 5 do
    begin
...
procedure AttackAction(bnum, mnum, level: integer);
var
  needprogress, step, range, AttAreaType, twice, t1, ax1, ay1, i, k, rnum, Aptitude, wpn,pzadd, tmp: integer;
  add: array[0..13] of integer;
begin
  //if (Brole[bnum].Team = 0) and (Brole[bnum].Auto = -1) then ShowMagicName(mnum);

  if (Brole[bnum].Team <> 0) and (not(istwice)) then
  begin
    //依据攻击范围进一步选择
    step := Rmagic[mnum].MoveDistance[level - 1];
    if CheckEquipSet(Rrole[Brole[bnum].rnum].equip[0], Rrole[Brole[bnum].rnum].equip[1],
  ...
  begin
    ...
    Brole[bnum].zhuangtai[5] := min(100, Brole[bnum].zhuangtai[5] + 60);
    Brole[bnum].zhuangtai[6] := min(100, Brole[bnum].zhuangtai[6] + 60);
    Brole[bnum].zhuangtai[10] := min(100, Brole[bnum].zhuangtai[10] + 60);

    Brole[bnum].zhuangtai[13] := min(100, Brole[bnum].zhuangtai[13] + 60);
    ...
    tmp:= Rrole[Brole[bnum].rnum].PhyPower;
    Rrole[Brole[bnum].rnum].PhyPower := min(100,Rrole[Brole[bnum].rnum].PhyPower + 10);
    Rrole_a[Brole[bnum].rnum].PhyPower :=Rrole_a[Brole[bnum].rnum].PhyPower + Rrole[Brole[bnum].rnum].PhyPower - tmp;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            max2 := max2 - 1;
            //test:= 'bnum:' + inttostr(bnum) + 'MenuStatus:' + inttostr(MenuStatus) + 'menustatus2:' + inttostr(menustatus2) + 'menu:' + inttostr(menu) + 'max0:' + inttostr(max0) + 'max2:' + inttostr(max2) + 'rst:' + inttostr(rst) + 'rst2:' + inttostr(rst2) ;
            //messagebox(0,@test[1],'4',MB_OK);
            ShowMagicMenu(bnum, MenuStatus, menustatus2, menu, max0, max2, rst, rst2);
          end
          else
          begin
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            hurt := trunc(hurt * (900 + Rzhaoshi[Brole[bnum].szhaoshi].gongji * (level - 1)) div 900);
            if (Rzhaoshi[Brole[n].szhaoshi].fangyu > 0) and (fmlev > 1) then
            begin
              hurt :=hurt - hurt * Rzhaoshi[Brole[n].szhaoshi].fangyu * (fmlev - 1) div 900;
            end;
            if random(3) = 1 then
        ...
        begin
          ...
          begin
            ...
            Rrole_a[Brole[n].rnum].Hurt:= Rrole_a[Brole[n].rnum].Hurt + Rrole[Brole[n].rnum].Hurt - tmp;
          end;
...
begin
  ...
  a1 := max(0.1, min((1 + a1) / (5 + att), zong));
  zong:=zong - a1 * (Rmagic[mnum].AttackModulus * 6 / p);
  w1 := max(0.1, min((1 + w1) / (5 + wpn1), zong));
  zong:=zong - w1 * (Rmagic[mnum].WeaponModulus * 2 / p);
  s1 := max(0.1, min((1 + s1) / (5 + spd1), zong));
  zong:=zong - s1 * (Rmagic[mnum].SpeedModulus * 2 / p);
  m1 := max(0.1, min((1 + m1) / (5 + mp1), zong));

  if p > 0 then
  begin
    if Rmagic[mnum].AttackModulus > 0 then
      Result := Result + trunc(mhurt * a1 * (Rmagic[mnum].AttackModulus * 6 / p));
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if not(angrycollect_pic.key) and (FileExists(BACKGROUND_file)) then
        begin
          grp := FileOpen(BACKGROUND_file, fmopenread);
          angrycollect_pic.pic := GetPngPic(grp, angrycollect_pic.num);
          angrycollect_pic.key:=true;
          fileclose(grp);
        end;
        drawpngpic(angrycollect_pic, 0, 0, 27 + Rrole[rnum].Angry, angrycollect_pic.pic.pic.h, x, y, 0)
      end
      else
...
begin
  ...
  begin
    ...
    if rnum >= 0 then
      ...
      begin
        Rrole_a[rnum].Exp := uint16(Rrole_a[rnum].Exp - LevelUplist[Rrole[rnum].Level - 1]);
        Rrole[rnum].Exp := uint16(Rrole[rnum].Exp - LevelUplist[Rrole[rnum].Level - 1]);
...
begin
  ...
  tmp:=Rrole[rnum].CurrentHP + round(((100 - Rrole[rnum].Hurt) * power(Rrole[rnum].MaxHP,0.90)) / 1200);
  if tmp > Rrole[rnum].MaxHP then tmp := Rrole[rnum].MaxHP;
  Rrole_a[rnum].CurrentHP:=Rrole_a[rnum].CurrentHP + tmp - Rrole[rnum].CurrentHP;
  Rrole[rnum].CurrentHP := tmp;
...
begin
  ...
  begin
    if (Brole[bnum].Team <> 0) or ((Brole[bnum].Team = 0) and (Brole[bnum].Auto in [1,2])) then
    begin
      if (random(100) < 70) and (AutoUseItem(bnum, 47, 1)) then
      begin
  ...
  begin
    ...
    begin
      if (random(100) < 60) and AutoUseItem(bnum, 52, 1) then
      begin
        FarthestMove(Mmx, Mmy, bnum);
        Ax := Mmx;
        Ay := Mmy;
        MoveAmination(bnum);
        AutoUseItem(bnum, 52, 0);
      end;
    end;
  end;
  ...
  begin
    if (Brole[bnum].Team <> 0) or ((Brole[bnum].Team = 0) and (Brole[bnum].Auto in [1])) then
    begin
      if (random(100) < 80) and AutoUseItem(bnum, 50, 1) then
      begin
        FarthestMove(Mmx, Mmy, bnum);
        Ax := Mmx;
        Ay := Mmy;
...
begin
  ...
  begin
    ...
    begin
      amount := amount + 1;
      setlength(a, amount);
      setlength(tmp, amount);
      a[amount - 1] := i;
      tmp[amount - 1]:=Brole[a[amount - 1]].Auto;
    end;
  end;
  menu := 0;
...
procedure zengyuan(mods, id, zyid: integer);
var
  i, i1, i2, j, j1, j2, len, n0, len0, len1, t, lenzt, lenzy, lenk, lrnum, k, tmplen: integer;
  find:boolean;
  pos1:Tposition;
begin
  ...
  t:=timetonum;
  lrnum:=-1;
  len0 := length(mpbdata[id].BTeam[0].rolearr);
  len1 := length(mpbdata[id].BTeam[1].rolearr);
  lenzt:= length(mpbdata[id].zaotui);
  for i:= lenzt -1 downto 0 do
  begin
    find:=false;
    for i1:=0 to len0 -1 do
    begin
      if mpbdata[id].BTeam[0].rolearr[i1].rnum = mpbdata[id].zaotui[i].rnum then
      begin
        find:=true;
        lrnum:= mpbdata[id].zaotui[i].rnum;
        ...
        dec(lenzt);
        for i2 := i1 to len0 - 2 do
        begin
          mpbdata[id].BTeam[0].rolearr[i2].rnum := mpbdata[id].BTeam[0].rolearr[i2 + 1].rnum;
          mpbdata[id].BTeam[0].rolearr[i2].snum := mpbdata[id].BTeam[0].rolearr[i2 + 1].snum;
          mpbdata[id].BTeam[0].rolearr[i2].isin := mpbdata[id].BTeam[0].rolearr[i2 + 1].isin;
          mpbdata[id].BTeam[0].rolearr[i2].mag := mpbdata[id].BTeam[0].rolearr[i2 + 1].mag;
          Rrole[mpbdata[id].BTeam[0].rolearr[i2].rnum].lsweizhi :=mpbdata[id].snum;
        end;
    ...
    begin
      ...
      begin
        if mpbdata[id].BTeam[1].rolearr[i1].rnum = mpbdata[id].zaotui[i].rnum then
        begin
          find:=true;
          lrnum:= mpbdata[id].zaotui[i].rnum;
          for i2 := i to lenzt -2 do
          ...
          setlength(mpbdata[id].zaotui,lenzt - 1);
          dec(lenzt);
          for i2 := i1 to len1 - 2 do
          begin
            mpbdata[id].BTeam[1].rolearr[i2].rnum := mpbdata[id].BTeam[1].rolearr[i2 + 1].rnum;
            mpbdata[id].BTeam[1].rolearr[i2].snum := mpbdata[id].BTeam[1].rolearr[i2 + 1].snum;
            mpbdata[id].BTeam[1].rolearr[i2].isin := mpbdata[id].BTeam[1].rolearr[i2 + 1].isin;
            mpbdata[id].BTeam[1].rolearr[i2].mag := mpbdata[id].BTeam[1].rolearr[i2 + 1].mag
          end;
          setlength(mpbdata[id].BTeam[1].rolearr,len1 -1);
          dec(len1);
        end;
        break;
      end;
  ...
  if lrnum >=0 then
  begin
    NewTalk(Rrole[lrnum].headnum, 207, -2, 1, 0, 0, 0, 1);
  end;
  lrnum:=-1;
  if zyid in[0,1] then
  begin
    ...
    while(k >= 0) do
    begin
      if (mpbdata[id].zengyuan[k].duizhang.rnum >= 0) and (t >= mpbdata[id].zengyuan[k].duizhang.day) then
      begin
        lenk:=length(mpbdata[id].BTeam[zyid].rolearr);
        setlength(mpbdata[id].BTeam[zyid].rolearr,lenk + 1);
        mpbdata[id].BTeam[zyid].rolearr[lenk].rnum := mpbdata[id].zengyuan[k].duizhang.rnum;
        mpbdata[id].BTeam[zyid].rolearr[lenk].snum := mpbdata[id].zengyuan[k].duizhang.snum;
        mpbdata[id].BTeam[zyid].rolearr[lenk].isin := mpbdata[id].zengyuan[k].duizhang.isin;
        mpbdata[id].BTeam[zyid].rolearr[lenk].mag := mpbdata[id].zengyuan[k].duizhang.mag;
        lrnum:=mpbdata[id].zengyuan[k].duizhang.rnum;
        inc(lenk);
        lenzy:=length(mpbdata[id].zengyuan[k].duiyuan);
        for i1:=0 to  lenzy - 1 do
        begin
          setlength(mpbdata[id].BTeam[zyid].rolearr,lenk + 1);
          mpbdata[id].BTeam[zyid].rolearr[lenk].rnum := mpbdata[id].zengyuan[k].duiyuan[i1].rnum;
          mpbdata[id].BTeam[zyid].rolearr[lenk].snum := mpbdata[id].zengyuan[k].duiyuan[i1].snum;
          mpbdata[id].BTeam[zyid].rolearr[lenk].isin := mpbdata[id].zengyuan[k].duiyuan[i1].isin;
          mpbdata[id].BTeam[zyid].rolearr[lenk].mag := mpbdata[id].zengyuan[k].duiyuan[i1].mag;
          lrnum:=mpbdata[id].zengyuan[k].duiyuan[i1].rnum;
          inc(lenk);
        end;
        for i:= k to mpbdata[id].ZYlen - 2 do
        begin
          move(mpbdata[id].zengyuan[i + 1].duizhang,mpbdata[id].zengyuan[i].duizhang,sizeof(TBZYRole));
          tmplen:=length(mpbdata[id].zengyuan[i + 1].duiyuan);
          setlength(mpbdata[id].zengyuan[i].duiyuan,tmplen);
          for j:= 0 to tmplen - 1 do
          begin
            mpbdata[id].zengyuan[i].duiyuan[j] := mpbdata[id].zengyuan[i + 1].duiyuan[j];
          end;
        end;
  ...
  if lrnum >=0 then
  begin
    NewTalk(Rrole[lrnum].headnum, 206, -2, 0, 0, 0, 0, 1);
  end;
...
function courthurt(bnum1, bnum2, mnum, level: integer; var tzhaoshi1: integer): integer;
var
  a1, a2, rnum1, rnum2, znum, i, tmp:integer;
  thurt, thurt0, mthurt, p, p1, tmp1: double;
begin
  Result := 0;
  rnum1 := Brole[bnum1].rnum;
  rnum2 := Brole[bnum2].rnum; // 敌人的rnum
  // 计算评分, 若为攻击型, 按照原方法评分
  if (Rmagic[mnum].MagicType < 7) and (Brole[bnum1].team <> Brole[bnum2].team) then
  begin
    mthurt := 0;
    znum := 0;
    for i := 0 to 9 do
      if Rrole[rnum1].LMagic[Rrole[rnum1].jhMagic[i]] = mnum then
      begin
        znum := Rrole[rnum1].jhMagic[i];
        break;
      end;
    thurt0 := CalHurtValue(bnum1, bnum2, mnum, level);
    p := 1; // 所有对该值操作，最终结果为伤害值乘以该值
    if Rrole[rnum2].MaxHP > 0 then
    begin
      if (Rrole[rnum2].CurrentHP - thurt0) <= 0 then
      begin
        p :=p + 1.5 + 1.5 * min(1, (thurt0 - Rrole[rnum2].CurrentHP) / (Rrole[rnum2].MAXHP * 0.2));
        // 需修改 要点：伤害溢出，但是由于最终伤害结果不一定精确，所以少量溢出是能够杀敌的保证，但是溢出太多也没有什么意义
        // 修改为如果少量溢出，评分乘以系数1.25
      end
      else
      begin
        p := p + 1.5 * power(0.988,(Rrole[rnum2].CurrentHP - thurt0) * 1000 / Rrole[rnum2].MaxHP)
        // 需修改，要点：能否重伤敌人，可能重伤的权重更高
        // 修改为如果敌人剩余生命较少，乘以一个系数，该系数最大值是0.2
      end;
      if Rmagic[mnum].MaxPeg > 0 then
      begin
        p := p + 0.2 * p * exp((Rmagic[mnum].MinPeg + (Rmagic[mnum].MaxPeg - Rmagic[mnum].MinPeg) * (level - 1)) *
          (100 - Brole[bnum2].zhuangtai[0]) / (Rmagic[mnum].MaxPeg * 1000) - 1);
        // 点穴的加成 乘方的意义在于提升大数值的权重
        // 改用exp后面的算式不会超过1，最大提升评分20%
      end;
      if Rmagic[mnum].Poision > 0 then
      begin
        p := p + 0.2 * p * exp(Rmagic[mnum].Poision * (level - 1) *
          (100 - Brole[bnum2].zhuangtai[0]) / 1000 - 1);
        // 带毒的加成 乘方的意义在于提升大数值的权重
        // 改用exp后面的算式不会超过1，最大提升评分20%
      end;
      if Rmagic[mnum].MaxInjury > 0 then
      begin
        p := p + 0.2 * p * exp(Rmagic[mnum].MinInjury + (Rmagic[mnum].MaxInjury - Rmagic[mnum].MinInjury) * (level - 1) *
          (100 - Brole[bnum2].zhuangtai[0]) / (Rmagic[mnum].MaxInjury * 1000) - 1);
        // 点穴的加成 乘方的意义在于提升大数值的权重
        // 改用exp后面的算式不会超过1，最大提升评分20%
      end;
    end
    ...
    for a1 := 0 to 4 do // 5个招式
    begin
      p1:=p;
      if Rmagic[mnum].zhaoshi[a1] <= 0 then
        break;
      if ((Rrole[rnum1].lzhaoshi[znum] and (1 shl a1)) = 0) or (Rzhaoshi[Rmagic[mnum].zhaoshi[a1]].ygongji < 1) then
        // 是否学会及攻击招式
        continue;
      p1 := p1 + (Rzhaoshi[Rmagic[mnum].zhaoshi[a1]].gongji * level / 1000); // “攻击加成”特效
      for a2 := 0 to 23 do
      begin
        if (Rzhaoshi[Rmagic[mnum].zhaoshi[a1]].texiao[a2].x < 0) then
          break;
        if (Rzhaoshi[Rmagic[mnum].zhaoshi[a1]].texiao[a2].x <= 9) and
          (Rzhaoshi[Rmagic[mnum].zhaoshi[a1]].texiao[a2].y > 0) then // 削弱特效
        begin
          tmp:=Rzhaoshi[Rmagic[mnum].zhaoshi[a1]].texiao[a2].y;
          p1 := p1 + tmp * 1.5 * power(0.995,Brole[bnum2].zhuangtai[Rzhaoshi[Rmagic[mnum].zhaoshi[a1]].texiao[a2].x] - tmp)/100;
             // 要点：1溢出；2大数值权重；
        end
        else if (Rzhaoshi[Rmagic[mnum].zhaoshi[a1]].texiao[a2].x >= 10) and
          (Rzhaoshi[Rmagic[mnum].zhaoshi[a1]].texiao[a2].x <= 19) and
          (Rzhaoshi[Rmagic[mnum].zhaoshi[a1]].texiao[a2].y > 0) then
        begin // 增加自身状态特效,权重大约是2倍-3倍
          tmp:=min(100 - Brole[bnum1].zhuangtai[Rzhaoshi[Rmagic[mnum].zhaoshi[a1]].texiao[a2].x - 5],
           Rzhaoshi[Rmagic[mnum].zhaoshi[a1]].texiao[a2].y);
          p1 := p1 + tmp * (-0.00032 * power(tmp, 2.01) + 0.028 * tmp + 1) / 100;
        end // 要点：1溢出；
        else if (Rzhaoshi[Rmagic[mnum].zhaoshi[a1]].texiao[a2].x >= 30) and
          (Rzhaoshi[Rmagic[mnum].zhaoshi[a1]].texiao[a2].x <= 39) and
          (Rzhaoshi[Rmagic[mnum].zhaoshi[a1]].texiao[a2].y > 0) then // 恢复特效,权重大约是10倍
        begin
          tmp:=min(100 - Brole[bnum1].zhuangtai[Rzhaoshi[Rmagic[mnum].zhaoshi[a1]].texiao[a2].x - 30],
            Rzhaoshi[Rmagic[mnum].zhaoshi[a1]].texiao[a2].y);
          p1 := p1 + tmp * 15 * power(0.995,Brole[bnum1].zhuangtai[Rzhaoshi[Rmagic[mnum].zhaoshi[a1]].texiao[a2].x - 30] + tmp)/100;; // 要点:1溢出
        end
        else if (Rzhaoshi[Rmagic[mnum].zhaoshi[a1]].texiao[a2].x >= 50) and
          (Rzhaoshi[Rmagic[mnum].zhaoshi[a1]].texiao[a2].x <= 59) and
          (Rzhaoshi[Rmagic[mnum].zhaoshi[a1]].texiao[a2].y > 0) then // 减弱自身状态的特效,负效果
        begin
          tmp:=min((Brole[bnum1].zhuangtai[Rzhaoshi[Rmagic[mnum].zhaoshi[a1]].texiao[a2].x - 50] -
            Brole[bnum1].lzhuangtai[Rzhaoshi[Rmagic[mnum].zhaoshi[a1]].texiao[a2].x - 50]),
            Rzhaoshi[Rmagic[mnum].zhaoshi[a1]].texiao[a2].y);
          p1 := p1 - tmp * 1.5 * power(0.995,Brole[bnum1].zhuangtai[Rzhaoshi[Rmagic[mnum].zhaoshi[a1]].texiao[a2].x - 50] - tmp)
           * Rrole[Brole[bnum1].rnum].CurrentHP / Rrole[Brole[bnum1].rnum].MAXHP / 100;
        end // 要点:1溢出;2自己濒临死亡的时候状态数值的价值降低
        else if (Rzhaoshi[Rmagic[mnum].zhaoshi[a1]].texiao[a2].x >= 70) and
          (Rzhaoshi[Rmagic[mnum].zhaoshi[a1]].texiao[a2].x <= 89) and
          (Rzhaoshi[Rmagic[mnum].zhaoshi[a1]].texiao[a2].y > 0) then // 内功配套效果
          for i := 0 to 9 do
          begin
            if (Rmagic[Rrole[rnum1].LMagic[Rrole[rnum1].Gongti]].MoveDistance[i] = Rzhaoshi[Rmagic[mnum].zhaoshi[a1]].texiao[a2].x) then // 查找当前内功是否与招式有配套效果,其中MoveDistance的实际意义转变为内功特效
            begin
              p1 := p1 + (Rzhaoshi[Rmagic[mnum].zhaoshi[a1]].texiao[a2].y) / 100;
            end;
          end;
      end;
      thurt := thurt0 * p1;
  ...
  begin
    // 医疗的评分, 基础分值等于医疗值 * 2
    p := 0.8;
    thurt0 := CalMedValue(bnum1, bnum2, mnum, level);
    // 如果为医疗人物，则基础分值*3
    if Rrole[rnum1].Attack > 0 then
    begin
      p := p + min(10,0.2 * GetRoleMedcine(Brole[bnum1].rnum, True) / GetRoleAttack(Brole[bnum1].rnum, True));
    end
    else
      p:= p + 3;
    if Rrole[rnum1].PhyPower < 50 then
      p := p - 0.2 * 50 / max(10,Rrole[rnum1].PhyPower);
    // 目标人物体力低系数变大
    tmp:= min(round(thurt0),Rrole[rnum2].MaxHP - Rrole[rnum2].CurrentHP);
    tmp1:=(Rrole[rnum2].CurrentHP * 1000 / Rrole[rnum2].MaxHP);
    p := p + tmp * 1000 /(Rrole[rnum2].MaxHP * Rrole[rnum2].MaxHP) *
     (0.000022*power(tmp1,2)-0.0171*tmp1+5-0.0000000099*power(tmp1,3));
    for a1 := 0 to 4 do // 5个招式
    begin
      p1:=p;
      if Rmagic[mnum].zhaoshi[a1] <= 0 then
      ...
      begin
        if (Rzhaoshi[Rmagic[mnum].zhaoshi[a1]].texiao[a2].x < 0) then
          break;
        if (Rzhaoshi[Rmagic[mnum].zhaoshi[a1]].texiao[a2].x >= 10) and
          (Rzhaoshi[Rmagic[mnum].zhaoshi[a1]].texiao[a2].x <= 19) and
          (Rzhaoshi[Rmagic[mnum].zhaoshi[a1]].texiao[a2].y > 0) then
        begin // 增加自身状态特效,权重大约是2倍-3倍
          tmp:=min(100 - Brole[bnum1].zhuangtai[Rzhaoshi[Rmagic[mnum].zhaoshi[a1]].texiao[a2].x - 5],
...
begin
  ...
  if Rmenpai[mpnum].zmr = rnum then
  begin
    result:=true;
    exit;
  end;
  for i:=0 to 1 do
  begin
    if Rmenpai[mpnum].zhiwu[i] = rnum then
    begin
      result:=true;
      break;
    end;
