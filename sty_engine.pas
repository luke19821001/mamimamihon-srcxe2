...
function TalktoWidestring(num: integer; fgrp, fidx: AnsiString): WideString;
...
// 判断是否盟友
function ismengyou(mpnum1, mpnum2: integer): boolean;
...
begin
  ...
  begin
    ...
    begin
      ...
      while ((wdate[1] > 60) or (wdate[2] > 12) or (wdate[3] > 30) or
        (wdate[4] > 47)) do
      begin
        if (wdate[4] > 47) then
        begin
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if ((t - DData[snum, eventnum, 11]) mod DData[snum, eventnum, 13]) <
          DData[snum, eventnum, 12] then
          Result := True;
      end
      else if (t - DData[snum, eventnum, 11]) < DData[snum, eventnum, 12] then
        Result := True;
    end;
...
function TimeToNum: integer;
begin
  Result := (wdate[0] - 1) * 360 * 60 + (wdate[1] - 1) * 360 + (wdate[2] - 1) *
    30 + wdate[3];
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      //if (debug = 1) then
      begin
        writeln(debugfile, IntToStr(wdate[1]) + '年' + IntToStr(wdate[2]) + '月' +
          IntToStr(wdate[3]) + '日');
        flush(debugfile);
        if isconsole then
          writeln(IntToStr(wdate[1]) + '年' + IntToStr(wdate[2]) + '月' +
            IntToStr(wdate[3]) + '日');
        writeln(debugfile, 'MyDiziLevelUp');
        flush(debugfile);
      end;
      MyDiziLevelUp;
      //if (debug = 1) then
      begin
        writeln(debugfile, 'daily');
        flush(debugfile);
      end;
      daily;
      //if (debug = 1) then
      begin
        writeln(debugfile, 'checktimeout');
        flush(debugfile);
      end;
      CheckTimeout;
      //if (debug = 1) then
      begin
        writeln(debugfile, 'dalyrestore');
        flush(debugfile);
      end;
      dalyrestore;
      if wdate[3] = 2 then
      begin
        //if (debug = 1) then
        begin
          writeln(debugfile, 'initialmpmagic');
          flush(debugfile);
        end;
        initialmpmagic;
      end
      else if wdate[3] = 14 then
      begin
        //if (debug = 1) then
        begin
          writeln(debugfile, 'newdizijoin');
          flush(debugfile);
        end;
        newdizijoin;
      end
      else if wdate[3] = 15 then
      begin
        //if (debug = 1) then
        begin
          writeln(debugfile, 'jisuanmenpaidata');
          flush(debugfile);
        end;
        jisuanmenpaidata;
      end
      else if wdate[3] = 16 then
      begin
       // if (debug = 1) then
        begin
          writeln(debugfile, 'airenming');
          flush(debugfile);
        end;
        airenming;
      end
      ...
      begin
        //if (debug = 1) then
        begin
          writeln(debugfile, 'initialMPdiaodu');
          flush(debugfile);
        end;
        initialMPdiaodu;
      end
...
begin
  ...
  for i := 1 to length(Rrole) - 1 do
    ...
    begin
      ...
      if (Rrole[i].dtime <= 0) and (Rrole[i].MenPai > 0) and
        (Rscene[Rrole[i].weizhi].MenPai <> Rrole[i].MenPai) and
        (Rmenpai[Rrole[i].MenPai].zongduo > 0) then
      begin
  ...
  for i := 1 to length(Rrole) - 1 do
    if (Rrole[i].MenPai > 0) and (Rrole[i].weizhi > 0) and (Rrole[i].dtime < 1)
    then
    begin
      anpai(i);
    end;
...
begin
  ...
  for i := 0 to length(DData) - 1 do
    for j := 0 to 399 do
    begin
      if (DData[i, j, 11] >= 0) and (DData[i, j, 14] > 0) and
        (DData[i, j, 15] = 0) then
      begin
      ...
      if (DData[i, j, 0] > 9) and (DData[i, j, 11] >= 0) and
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          str := str + '在' + gbktounicode(@Rscene[mpbdata[i].snum].Name)
            + '的爭鬥開始！';
          addtips(str);

          if (mpbdata[i].attmp = Rrole[0].MenPai) and
          ...
          else if (mpbdata[i].defmp = Rrole[0].MenPai) and
            (Rrole[0].weizhi = mpbdata[i].snum) then
          begin
            NewTalk(Rrole[mpbdata[i].bteam[2].rolearr[0].rnum].headnum, 204, -2,
              0, 0, 0, 0, 1);
            timetompbattle(i, 1);
            gotommap(-1, -1);
            exit;
  ...
  begin
    ...
    else if (TimeTrigger.adds[i].types = 1) and
...
begin
  ...
  begin
    ...
    begin
      if mpbdata[id].bteam[2 + mods].rolearr[i - len0].rnum = 0 then
      begin
      ...
      mpbdata[id].bteam[0].rolearr[i].rnum := mpbdata[id].bteam[2 + mods]
        .rolearr[i - len0].rnum;
      mpbdata[id].bteam[0].rolearr[i].snum :=
        Rrole[mpbdata[id].bteam[2 + mods].rolearr[i - len0].rnum].weizhi;
    ...
    for i := 0 to len - 1 do
    begin
      mpbdata[id].bteam[1].rolearr[i].rnum := mpbdata[id].bteam[3 - mods]
        .rolearr[i].rnum;
      mpbdata[id].bteam[1].rolearr[i].snum :=
        Rrole[mpbdata[id].bteam[3 - mods].rolearr[i].rnum].weizhi;
...
procedure MyDiziLevelUp;
var
  i, j, j1, j2, k, k1, k2, n, mpnum, tm, ltmp, Aptitude, miji, tneigong,
    zhiwujc6, zhiwujc7, zhiwujc9, mnum, nolw, lwstatus, nnum, lwnum, p, mexp,
    tmp0: integer;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          mexp := round((Rrole[i].level div 3 + 10) * (100 + random(100)) *
            (100 + zhiwujc6) / 30000);
          ...
          Rrole_a[i].maglevel[tmp[tm]] := Rrole_a[i].maglevel[tmp[tm]] +
            Rrole[i].maglevel[tmp[tm]] - tmp0;
          if ((Rrole[i].maglevel[tmp[tm]] div 100) > (ltmp div 100)) then
          begin
            EatOneItem(i, Rmagic[mnum].miji, False);
            str := '【門派公告】' + gbktounicode(@Rrole[i].Name) + '的' +
              gbktounicode(@(Rmagic[Rrole[i].lmagic[tmp[tm]]].Name)) + '升級為' +
          ...
          if nnum > 0 then
          begin
            if CheckEquipSet(Rrole[i].equip[0], Rrole[i].equip[1],
              Rrole[i].equip[2], Rrole[i].equip[3]) = 2 then
              Aptitude := 100
            else
              Aptitude := Rrole[i].Aptitude;
            if Ritem[Rmagic[mnum].miji].NeedAptitude >= 0 then
              Aptitude := 2 + Aptitude - Ritem[Rmagic[mnum].miji].NeedAptitude
            else
              Aptitude := 2 - 2 * (Ritem[Rmagic[mnum].miji].NeedAptitude +
                Aptitude);
            ...
            begin
              ...
              begin
                ...
                Rrole[i].lzhaoshi[tmp[tm]] := Rrole[i].lzhaoshi[tmp[tm]] or
                  (1 shl j2);
                Rrole_a[i].lzhaoshi[tmp[tm]] := Rrole_a[i].lzhaoshi[tmp[tm]] +
                  Rrole[i].lzhaoshi[tmp[tm]] - tmp0;
                str := gbktounicode(@Rrole[i].Name);
                str := str + '領悟';
                str := str + gbktounicode
                  (@Rzhaoshi[Rmagic[mnum].zhaoshi[j2]].Name);
      ...
      begin
        ...
        else if ((Rmenpai[mpnum].zmr <> 0) or (Rmenpai[mpnum].zhiwu[6] >= 0))
          and ((random(7500) < Rrole[i].msq) or (Rrole[i].lmagic[0] < 1)) then
        begin
          ...
          begin
            ...
            begin
              p := 0;
              if Rmpmagic[mpnum][k] <= 0 then
                break;
              if not(CanEquip(i, Rmagic[Rmpmagic[mpnum][k]].miji)) then
                continue;
              for k1 := 0 to 29 do
              begin
                if Rrole[i].lmagic[k1] = Rmpmagic[mpnum][k] then
                  break;
                if Rrole[i].lmagic[k1] <= 0 then
                begin
                  Rrole_a[i].lmagic[k1] := Rrole_a[i].lmagic[k1] +
                    Rmpmagic[mpnum][k] - Rrole[i].lmagic[k1];
        ...
        begin
          ...
          begin
            if Rrole[i].lmagic[k] <= 0 then
              break;
            if Rmagic[Rrole[i].lmagic[k]].Ismichuan > 0 then
              continue;
            for k1 := 0 to 399 do
            begin
              if Rrole[i].lmagic[k] = Rmpmagic[mpnum][k1] then
                break;
              if Rmpmagic[mpnum][k1] < 0 then
              begin
                for k2 := 0 to MAX_ITEM_AMOUNT - 1 do
                begin
                  if Ritemlist[k2].number = Rmagic[Rrole[i].lmagic[k]].miji then
                    break;
          ...
          begin
            tm := random(n);
            if (randomf1 < 1 + Aptitude * lv[tm] * (100 + zhiwujc9) div 270)
            then
            begin
      ...
      begin
        ...
        begin
          if Rrole[i].lmagic[k] <= 0 then
            break;
          if (Rmagic[Rrole[i].lmagic[k]].magictype = 5) and
            ((Rrole[i].maglevel[k] div 100) < Rmagic[Rrole[i].lmagic[k]]
            .MaxLevel) then
          begin
            tmp[n] := k;
        ...
        begin
          ...
          ltmp := Rrole[i].maglevel[tmp[tm]];
          Inc(Rrole[i].maglevel[tmp[tm]],
            round((Rrole[i].level div 3 + 10) * (100 + random(100)) *
            (100 + zhiwujc9) / 50000));
          if Rrole[i].maglevel[tmp[tm]] div 100 > Rmagic[Rrole[i].lmagic[tm]].MaxLevel
          then
            Rrole[i].maglevel[tmp[tm]] := Rmagic[Rrole[i].lmagic[tm]]
              .MaxLevel * 100;
          Rrole_a[i].maglevel[tmp[tm]] := Rrole_a[i].maglevel[tmp[tm]] +
            Rrole[i].maglevel[tmp[tm]] - tmp0;
          if ((Rrole[i].maglevel[tmp[tm]] div 100) > (ltmp div 100)) then
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              if (Rmagic[Rrole[i].lmagic[k]].magictype <> 5) then
              begin
                if (Rmagic[Rrole[i].lmagic[k]].teshu[0] = 0) and
                  ((Rmagic[Rrole[i].lmagic[k]].teshumod[0] = -1)) then
                  continue;
                for k1 := 0 to 9 do
                begin
                  ...
                  begin
                    if randomf1 <
                      (1 + (1 + Aptitude) * lv[tm] * (100 + zhiwujc9) div 360)
                    then
                    begin
                      ...
                      str := '【門派公告】' + gbktounicode(@Rrole[i].Name) + '將' +
                        ...
                        gbktounicode(@(Rmagic[Rrole[i].lmagic[k]].Name)
                        ) + '融會貫通';
                      addtips(str);
                      key := True;
                    end;
      ...
      begin
        ...
        begin
          ...
          begin
            instruct_32(k, 1);
            str := '【門派公告】' + gbktounicode(@Rrole[i].Name) + '煉製出' +
      ...
      begin
        ...
        begin
          ...
          begin
            instruct_32(k, 1);
            str := '【門派公告】' + gbktounicode(@Rrole[i].Name) + '製造出' +
              gbktounicode(@(Ritem[k].Name));
            addtips(str);
          end;
        end;
...
procedure EnemyDiziLevelUp;
var
  i, j, j1, j2, k, k1, mp0, n, n1, n2, tm, tm1, tm2, mpnum, tneigong, Aptitude,
    mnum, nolw, lwstatus, nnum, lwnum, p, ltmp: integer;
  tmp0: integer;
...
begin
  ...
  for i := 0 to length(Rrole) - 1 do
  begin
    if (Rrole[i].MenPai <> mp0) and (Rrole[i].MenPai > 0) and
      (Rrole[i].MenPai < 40) then
    begin
      ...
      begin
        ...
        if (Rmagic[Rrole[i].lmagic[k]].magictype = 5) and
          ((Rrole[i].maglevel[k] div 100) < Rmagic[Rrole[i].lmagic[k]].MaxLevel)
        then
        begin
          tmp2[n2] := k;
          Inc(n2);
      ...
      begin
        for j := 0 to 29 do
        begin
          if (random(100) > Rrole[i].lwq) and (random(Rmenpai[mpnum].dizi) > 7)
          then
            continue;
          tm := random(n);
          ...
          begin
            ...
            if Ritem[Rmagic[mnum].miji].NeedAptitude >= 0 then
              ...
              Aptitude := 2 - (Ritem[Rmagic[mnum].miji].NeedAptitude +
                Aptitude);
      ...
      begin
        for j := 0 to 29 do
        begin
          if (random(100) > Rrole[i].msq) and (random(Rmenpai[mpnum].dizi) > 7)
          then
            continue;
          tm1 := random(n1);
          ...
          Rrole_a[i].maglevel[tmp1[tm1]] := Rrole_a[i].maglevel[tmp1[tm1]] +
            Rrole[i].maglevel[tmp1[tm1]] - tmp0;
      ...
      begin
        for j := 0 to 29 do
        begin
          if (random(100) > Rrole[i].msq) and (random(Rmenpai[mpnum].dizi) > 1)
          then
            continue;
          ...
          if Rrole[i].maglevel[tmp2[tm2]] >
            (Rmagic[Rrole[i].lmagic[tmp2[tm2]]].MaxLevel * 100) then
            Rrole[i].maglevel[tmp2[tm2]] := Rmagic[Rrole[i].lmagic[tmp2[tm2]]]
              .MaxLevel * 100;
          Rrole_a[i].maglevel[tmp2[tm2]] := Rrole_a[i].maglevel[tmp2[tm2]] +
            Rrole[i].maglevel[tmp2[tm2]] - tmp0;
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                begin
                  if random(10000) <
                    (1 + ((1 + Aptitude) * lv[tm] * mpzyjc(Rrole[i].MenPai)
                    div 360)) then
                  begin
                    Rmagic[Rrole[i].lmagic[k]].teshu[k1] := tneigong;
                    Rmagic[Rrole[i].lmagic[k]].teshumod[k1] := mpnum;
      ...
      if (Rrole[i].MenPai > 0) and (Rrole[i].MenPai < 40) and
        ((random(300) < Rrole[i].msq * Rmenpai[Rrole[i].MenPai].czsd *
        mpzyjc(Rrole[i].MenPai) div 10000) or (Rrole[i].lmagic[0] < 1)) then
      begin
        for k := 0 to 399 do
        begin
          if (Rmpmagic[mpnum][k] < 0) then
          ...
          begin
            ...
            if Rrole[i].lmagic[k1] <= 0 then
            begin
              Rrole_a[i].lmagic[k1] := Rrole_a[i].lmagic[k1] + Rmpmagic[mpnum]
                [k] - Rrole[i].lmagic[k1];
              Rrole[i].lmagic[k1] := Rmpmagic[mpnum][k];
              break;
            end;
...
begin
  flag := True;
  j := Rmenpai[Rrole[num].MenPai].neigong
    [random(GetMenpaiMagicNumber(Rrole[num].MenPai))];
...
var
  ...
  pp: array [0 .. 200] of array [0 .. 1] of AnsiChar; // 与输入拼音匹配的汉字
  houxuan: array [0 .. 8] of array [0 .. 1] of AnsiChar;
  hxtmp: array [0 .. 3] of AnsiChar; // 选字状态下，候选字符
...
begin
  ...
  begin
    ...
    begin
      if (k mod 2) = 0 then
      begin
        pyw[k div 2].hanzi[k1] := AnsiChar(0);
        pyw[k div 2].hanzi[k1 + 1] := AnsiChar(0);
      end
      else
      begin
        pyw[k div 2].pinyin[k1] := AnsiChar(0);
        pyw[k div 2].pinyin[k1 + 1] := AnsiChar(0);
      end;
      i := i + 1;
      k1 := 0;
      Inc(k);
    ...
    begin
      pyw[k div 2].hanzi[k1] := AnsiChar(fdata[i]);
      Inc(k1);
    end
    else if (k mod 2) = 1 then
    begin
      pyw[k div 2].pinyin[k1] := AnsiChar(fdata[i]);
      Inc(k1);
    end;
    Inc(i);
  end;
  ...
  begin
    ...
    case event.type_ of
      ...
      SDL_KEYUP:
        begin
          // 输入a-z
          if ((event.key.keysym.sym > 96) and (event.key.keysym.sym < 123) and
            (state < 2)) then
          begin
            if state = 0 then
            begin
            ...
            npy := AnsiChar(event.key.keysym.sym);
            py := py + npy;
          end;
          // 输入BACKSPACE，删除
          ...
          begin
            ...
            begin
              for i := 1 to 9 do
                if ((Name[i] = AnsiChar(0)) and (Name[i - 1] = AnsiChar(0)) and
                  (i mod 2 = 1)) then
                  break;
              if i > 2 then
              begin
          ...
          begin
            ...
            for i := 0 to 8 do
            begin
              if ((pp[i][0] <> AnsiChar(0)) or (pp[i][1] <> AnsiChar(0))) then
              begin
                houxuan[i][0] := pp[i][0];
                houxuan[i][1] := pp[i][1];
              end;
          ...
          // 输入-=，选字翻页
          if ((event.key.keysym.sym = SDLK_MINUS) or
            (event.key.keysym.sym = SDLK_EQUALS) and (state = 2)) then
          begin
            if (event.key.keysym.sym = SDLK_MINUS) and (ye > 0) then
              ye := ye - 1;
            if ((pp[9 * (ye + 1)][0] <> AnsiChar(0)) or
              (pp[9 * (ye + 1)][1] <> AnsiChar(0))) and
              (event.key.keysym.sym = SDLK_EQUALS) then
              ye := ye + 1;
            for i := 0 to 8 do
            begin
              if (pp[i + 9 * ye][0] <> AnsiChar(0)) or
                (pp[i + 9 * ye][1] <> AnsiChar(0)) then
              begin
          ...
          // 输入1-9，选字
          if ((event.key.keysym.sym > 48) and (event.key.keysym.sym < 58) and
            ((state = 2) or (state = 3))) then
          begin
            for i := 1 to 9 do
              if (Name[i] = AnsiChar(0)) and (Name[i - 1] = AnsiChar(0)) and
                (i mod 2 = 1) then
    ...
    for i := 0 to 8 do
    begin
      if (houxuan[i][0] = AnsiChar(0)) and (houxuan[i][1] = AnsiChar(0)) then
        break
      else
      begin
...
begin
  ...
  begin
    ...
    for i1 := 0 to 29 do
    begin
      if ((Rrole[i].lmagic[i1] <= 0) or (n > 9)) then
        break
            ...
            begin
              ...
              begin
                if (Rmagic[Rrole[i].lmagic[i1]].teshumod[0] = -1) or
                  ((Rmagic[Rrole[i].lmagic[i1]].teshu[j1] = Rrole[i].lmagic
                  [Rrole[i].gongti]) and
                  ((Rmagic[Rrole[i].lmagic[i1]].teshumod[j1] = 0) or
                  (Rmagic[Rrole[i].lmagic[i1]].teshumod[j1] = Rrole[i].MenPai)))
                then
                begin
                  k := 1;
                  break;
...
//MODS不等於0 計算人物武功強力值 1計算強力值的時候內功不乘方
function RoleValue(rnum,mods: integer): integer;
var
  ...
  begin
    ...
    begin
      value := Rmagic[tgongti[i]].AddHP[tgtlev[i]] * 2 + Rmagic[tgongti[i]]
        .AddMP[tgtlev[i]] + Rmagic[tgongti[i]].AddAtt[tgtlev[i]] * 10 +
        Rmagic[tgongti[i]].AddDef[tgtlev[i]] * 10 + Rmagic[tgongti[i]].AddSpd
        [tgtlev[i]] * 10;
      value := value + Rmagic[tgongti[i]].AddMedcine * 5 + Rmagic[tgongti[i]]
        .AddUsePoi * 5 + Rmagic[tgongti[i]].AddMedPoi * 5 + Rmagic[tgongti[i]]
      ...
      begin
        if (Rmagic[tgongti[i]].MoveDistance[i1] >= 40) and
          (Rmagic[tgongti[i]].MoveDistance[i1] < 50) then
        begin
          value := value * (200 + Rmagic[tgongti[i]].AttDistance[i1]) / 200;
        end
        else if (Rmagic[tgongti[i]].MoveDistance[i1] >= 60) and
          (Rmagic[tgongti[i]].MoveDistance[i1] < 70) then
        begin
          value := value * (100 + Rmagic[tgongti[i]].AttDistance[i1]) / 100;
        end
        else if (Rmagic[tgongti[i]].MoveDistance[i1] >= 50) and
          (Rmagic[tgongti[i]].MoveDistance[i1] < 60) then
        begin
          value := value * (200 - Rmagic[tgongti[i]].AttDistance[i1]) / 200;
        end
      end;
      if Rmagic[tgongti[i]].BattleState > 0 then
        value := value + 1000;
      if mods <> 1 then
      ...
      begin
        ...
        begin
          ...
          begin
            if i2 > 0 then
            begin
              if (Rmagic[tmagic[i1]].teshu[i2] <= 0) then
                break;
            end;
            if (Rmagic[tmagic[i1]].teshumod[0] = -1) or
              ((Rmagic[tmagic[i1]].teshu[i2] = tgongti[i]) and
...
begin
  if Rmagic[mnum].magictype = 6 then
    value := 100 * mlev div 9
  else
    value := CalNewHurtValue(mlev, Rmagic[mnum].MinHurt, Rmagic[mnum].MaxHurt,
      Rmagic[mnum].HurtModulus);
  k := 5;
  for i := 0 to 4 do
  begin
...
begin
  ...
  for i := 0 to length(Ritem) - 1 do
  begin
    if (Ritem[i].ItemType = 1) and ((Ritem[i].rate = 0) or (Ritem[i].rate = -1))
    then
    begin
      for i1 := 0 to MAX_ITEM_AMOUNT - 1 do
      begin
...
begin
  ...
  begin
    if (Rrole[i].MenPai >= 0) and (Rrole[i].MenPai < 40) and
      (Rrole[i].dtime < 5) and (Rmenpai[Rrole[i].MenPai].zmr <> i) then
    begin
  ...
  begin
    if (k[i] > 0) and ((i <> Rrole[0].MenPai) or
      (Rmenpai[Rrole[0].MenPai].zmr <> 0)) then
    begin
...
begin
  ...
  begin
    ...
    begin
      if Rrole[arr[j]].level * Rrole[arr[j]].Repute > Rrole[key].level *
        Rrole[key].Repute then
      begin
        arr[j - 1] := arr[j];
        arr[j] := key;
      end;
...
begin
  ...
  begin
    ...
    for j := i + 1 to length(arr) - 1 do
    begin
      if Rrole[arr[j]].level * Rrole[arr[j]].Repute div (Rrole[arr[j]].bssx + 1
        - big) > Rrole[key].level * Rrole[key].Repute div
        (Rrole[key].bssx + 1 - big) then
      begin
        arr[j - 1] := arr[j];
        arr[j] := key;
      end;
...
function gettips: WideString; //从传闻队列中获取最早一条传闻
var
  i: integer;
begin
...
procedure settips; //设置（增加）传闻显示队列
var
  i: integer;
...
procedure dectips(num: integer); //从传闻显示队列中删除一条
var
  i: integer;
  tipsstring: WideString;
begin
  tipsstring := gettips;
  if (tipsstring <> '') and (num < Showtips.num) then //传闻队列还有值时，不删除，直接替换
  begin
  ...
  begin
    if showtips.SurCreated[num] then
    begin
      SDL_FreeSurface(Showtips.Sur[num]);
    end;
...
procedure addtips(str: WideString); //把字符串加入传闻队列
var
  i, k: integer;
  s1: AnsiString;
begin
  ...
  setlength(RStishi.stishi[RStishi.num - 1].talk,
    RStishi.stishi[RStishi.num - 1].talklen);
  for i := 0 to RStishi.stishi[RStishi.num - 1].talklen - 3 do
  begin
    RStishi.stishi[RStishi.num - 1].talk[i] := s1[i + 1];
  end;
  RStishi.stishi[RStishi.num - 1].talk[RStishi.stishi[RStishi.num - 1].talklen -
    2] := AnsiChar(0);
  RStishi.stishi[RStishi.num - 1].talk[RStishi.stishi[RStishi.num - 1].talklen -
    1] := AnsiChar(0);
  if length(tipsarr) <= 0 then
  begin
    setlength(tipsarr, 1);
    tipsarr[0] := str + '（' + IntToStr(wdate[2]) + '．' +
      IntToStr(wdate[3]) + '）';
    exit;
  end
  else
  begin
    ...
    tipsarr[k] := str + '（' + IntToStr(wdate[2]) + '．' +
      IntToStr(wdate[3]) + '）';
  end;
...
procedure jisuanmenpaidata;
var
  i, j, add1, tmp, tmp2, i1, i2, i3, mpnum, n, k: integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if Rmenpai[i].guanxi[j] >= 800 then
          Inc(n);
  ...
  begin
    zhiwujc[2] := Rrole[Rmenpai[mpnum].zhiwu[2]].Repute *
      Rrole[Rmenpai[mpnum].zhiwu[2]].level div 300;
    zhiwujc[4] := Rrole[Rmenpai[mpnum].zhiwu[2]].Repute *
      Rrole[Rmenpai[mpnum].zhiwu[2]].level div 600;
    zhiwujc[5] := Rrole[Rmenpai[mpnum].zhiwu[2]].Repute *
      Rrole[Rmenpai[mpnum].zhiwu[2]].level div 300;
  end;
  if Rmenpai[mpnum].zhiwu[3] >= 0 then
  begin
    zhiwujc[0] := Rrole[Rmenpai[mpnum].zhiwu[3]].Repute *
      Rrole[Rmenpai[mpnum].zhiwu[3]].level div 300;
    zhiwujc[4] := Rrole[Rmenpai[mpnum].zhiwu[3]].Repute *
      Rrole[Rmenpai[mpnum].zhiwu[3]].level div 600;
    zhiwujc[7] := Rrole[Rmenpai[mpnum].zhiwu[3]].Repute *
      Rrole[Rmenpai[mpnum].zhiwu[3]].level div 300;
  end;
  if Rmenpai[mpnum].zhiwu[4] >= 0 then
  begin
    zhiwujc[3] := Rrole[Rmenpai[mpnum].zhiwu[4]].Repute *
      Rrole[Rmenpai[mpnum].zhiwu[4]].level div 300;
    zhiwujc[4] := Rrole[Rmenpai[mpnum].zhiwu[4]].Repute *
      Rrole[Rmenpai[mpnum].zhiwu[4]].level div 600;
    zhiwujc[6] := Rrole[Rmenpai[mpnum].zhiwu[4]].Repute *
      Rrole[Rmenpai[mpnum].zhiwu[4]].level div 300;
  end;
  if Rmenpai[mpnum].zhiwu[5] >= 0 then
  begin
    zhiwujc[1] := Rrole[Rmenpai[mpnum].zhiwu[5]].Repute *
      Rrole[Rmenpai[mpnum].zhiwu[5]].level div 300;
    zhiwujc[4] := Rrole[Rmenpai[mpnum].zhiwu[5]].Repute *
      Rrole[Rmenpai[mpnum].zhiwu[5]].level div 600;
    zhiwujc[8] := Rrole[Rmenpai[mpnum].zhiwu[5]].Repute *
      Rrole[Rmenpai[mpnum].zhiwu[5]].level div 300;
  end;
  ...
  begin
    ...
    begin
      if i1 = mpnum then
      begin
        Inc(Rmenpai[i1].ziyuan[i2], Rmenpai[i1].aziyuan[i2] * (100 + zhiwujc[i2]
          ) div 100);
        ...
        begin
          ...
          begin
            if (Rrole[i3].MenPai = mpnum) and (Rrole[i3].dtime < 1000) and
              (Rrole[i3].zhongcheng >= 0) then
            begin
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if (Rrole[i1].MenPai = i) and (Rrole[i1].dtime <= 0) and
          (((Rrole[i1].weizhi >= 0) and (Rscene[Rrole[i1].weizhi].MenPai <> i))
          or (Rrole[i1].weizhi < 0)) then
        begin
          Rrole[i1].weizhi := Rmenpai[i].zongduo;
        end;
      end;
      if Rmenpai[i].zmr <> 0 then
      begin
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                begin
                  kg := i4;
                  if Rmenpai[mpdiaodu[i].mpnum].zhiwu[i4] = mpdiaodu[i].rnum[i2]
                  then
                  begin
                    kg:=-1;
                    break;
  ...
  begin
    ...
    begin
      for i1 := 0 to max0 - 1 do
        jgzhanli := jgzhanli + RoleValue(rolearr[i1], 1);
      setlength(rolearr, max0);
    end
    else
    begin
    ...
    begin
      ...
      begin
        ...
        begin
          if (mpdiaodu[i1].sce[i2].dmenpai >= 0) and
            (Rscene[mpdiaodu[i1].sce[i2].dsnum].inbattle = 0) and
          ...
          begin
            ...
            begin
              writeln(debugfile, '计算门派' + IntToStr(i) + '是否进攻门派' +
                ...
                IntToStr(mpdiaodu[i1].sce[i2].snum));
              flush(debugfile);
            end;
            // if random(400000) < power(k, 3) then
            ...
            begin
              ...
              for i3 := 0 to length(Rmenpai) - 1 do
              begin
              ...
              if max1 > 0 then
                for i3 := 0 to max1 - 1 do
                  fyzhanli := fyzhanli + RoleValue(rolearr1[i3], 1);
              if max2 > 0 then
              begin
              ...
              begin
                ...
                begin
                  ...
                  begin
                    t0 := Rrole[ZY_rolearr[i3].duizhang].dtime * 2 + 20;
                    if Rrole[ZY_rolearr[i3].duizhang].nweizhi = 16 then
                    begin
                      t0 := 20;
                    end;
                    if ismengyou(Rrole[ZY_rolearr[i3].duizhang].MenPai,
                      mpdiaodu[i1].sce[i2].dmenpai) then
                      t0 := t0 + 50;
                    fyzhanli := fyzhanli + RoleValue(ZY_rolearr[i3].duizhang,1)
                     * 15 div (20 + t0);
                    for i4 := 0 to ZY_rolearr[i3].len - 1 do
                    begin
                      fyzhanli := fyzhanli +
                        RoleValue(ZY_rolearr[i3].duiyuan[i4], 1) *
                        15 div (20 + t0);
                    end;
                  end;
                end;
             ...
             if (debug = 1) then
              begin
                writeln(debugfile, '实力判断：门派' + IntToStr(i) + '攻击力' +
                  ...
                  IntToStr(mpdiaodu[i1].sce[i2].dmenpai) + '防御力' +
                  IntToStr(fyzhanli));
                flush(debugfile);
              end;
              if (random(jgzhanli * Rmenpai[i].kzq) > (fyzhanli * 150)) and
                (max0 > 0) then
              begin
                kg:=-1;
                for i3 := 0 to 19 do
                ...
                str := str + '向' +
                  gbktounicode(@Rscene[mpdiaodu[i1].sce[i2].dsnum].Name)
                  + '發動攻擊！';
                addtips(str);
                battle_id := i3;
                mpbdata[i3].key := 1;
                mpbdata[i3].daytime := TimeToNum + 1 +
                  ...
                  Rscene[mpdiaodu[i1].sce[i2].dsnum].MainEntranceX1)) div 36;
                mpbdata[i3].attmp := i;
                ...
                begin
                  writeln(debugfile, '门派' + IntToStr(i) + '进攻门派' +
                    IntToStr(mpdiaodu[i1].sce[i2].dmenpai) + '场景' +
                    IntToStr(mpdiaodu[i1].sce[i2].snum) + '计算完毕');
                  flush(debugfile);
                end;
                break;
              end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        tmp := CalNewHurtValue(level - 1, Rmagic[mnum].MinHurt,
          Rmagic[mnum].MaxHurt, Rmagic[mnum].HurtModulus);
...
begin
  ...
  lenzt := length(mpbdata[id].zaotui);
  for i := lenzt - 1 downto 0 do
  begin
    find := False;
    ...
    begin
      ...
      begin
        ...
        find := True;
        for i2 := i to lenzt - 2 do
        begin
          mpbdata[id].zaotui[i2] := mpbdata[id].zaotui[i2 + 1];
        end;
        setlength(mpbdata[id].zaotui, lenzt - 1);
        Dec(lenzt);
        for i2 := i1 to len2 - 2 do
        begin
          mpbdata[id].bteam[2].rolearr[i2].rnum := mpbdata[id].bteam[2].rolearr
            [i2 + 1].rnum;
          mpbdata[id].bteam[2].rolearr[i2].snum := mpbdata[id].bteam[2].rolearr
            [i2 + 1].snum;
          mpbdata[id].bteam[2].rolearr[i1].isin := mpbdata[id].bteam[2].rolearr
            [i2 + 1].isin;
          mpbdata[id].bteam[2].rolearr[i2].mag := mpbdata[id].bteam[2].rolearr
            [i2 + 1].mag
        end;
        setlength(mpbdata[id].bteam[2].rolearr, len2 - 1);
        Dec(len2);
      end;
      break;
    end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          setlength(mpbdata[id].zaotui, lenzt - 1);
          Dec(lenzt);
          for i2 := i1 to len3 - 2 do
          begin
            mpbdata[id].bteam[3].rolearr[i1].rnum := mpbdata[id].bteam[3]
              .rolearr[i1 + 1].rnum;
            mpbdata[id].bteam[3].rolearr[i1].snum := mpbdata[id].bteam[3]
              .rolearr[i1 + 1].snum;
            mpbdata[id].bteam[3].rolearr[i1].isin := mpbdata[id].bteam[3]
              .rolearr[i1 + 1].isin;
            mpbdata[id].bteam[3].rolearr[i1].mag := mpbdata[id].bteam[3].rolearr
              [i1 + 1].mag
          end;
          setlength(mpbdata[id].bteam[3].rolearr, len3 - 1);
          Dec(len3);
        end;
        break;
      end;
  ...
  k := mpbdata[id].ZYlen - 1;
  while (k >= 0) do
  begin
    if (mpbdata[id].zengyuan[k].duizhang.rnum >= 0) and
      (t >= mpbdata[id].zengyuan[k].duizhang.day) then
    begin

      setlength(mpbdata[id].bteam[3].rolearr, len3 + 1);
      mpbdata[id].bteam[3].rolearr[len3].rnum := mpbdata[id].zengyuan[k]
        .duizhang.rnum;
      mpbdata[id].bteam[3].rolearr[len3].snum := mpbdata[id].zengyuan[k]
        .duizhang.snum;
      mpbdata[id].bteam[3].rolearr[len3].isin := mpbdata[id].zengyuan[k]
        .duizhang.isin;
      mpbdata[id].bteam[3].rolearr[len3].mag := mpbdata[id].zengyuan[k]
        .duizhang.mag;
      Inc(len3);
      lenzy := length(mpbdata[id].zengyuan[k].duiyuan);
      for i1 := 0 to lenzy - 1 do
      begin
        setlength(mpbdata[id].bteam[3].rolearr, len3 + 1);
        mpbdata[id].bteam[3].rolearr[len3].rnum := mpbdata[id].zengyuan[k]
          .duiyuan[i1].rnum;
      ...
      for i := k to mpbdata[id].ZYlen - 2 do
      begin
        move(mpbdata[id].zengyuan[i + 1].duizhang,
          mpbdata[id].zengyuan[i].duizhang, SizeOf(TBZYRole));
        tmplen := length(mpbdata[id].zengyuan[i + 1].duiyuan);
        setlength(mpbdata[id].zengyuan[i].duiyuan, tmplen);
        for j := 0 to tmplen - 1 do
        begin
          mpbdata[id].zengyuan[i].duiyuan[j] := mpbdata[id].zengyuan[i + 1]
            .duiyuan[j];
        end;
      end;
      setlength(mpbdata[id].zengyuan, mpbdata[id].ZYlen - 1);
      Dec(mpbdata[id].ZYlen);
    end;
    Dec(k);
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        Rrole[mpbdata[id].bteam[2].rolearr[i2].rnum].weizhi :=
          mpbdata[id].bteam[2].rolearr[i2].snum;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if tmp = -1 then
        begin
          HurtValueAI(mpbdata[id].bteam[3].rolearr[i].rnum,
            mpbdata[id].bteam[2].rolearr[i1].rnum,
            Rrole[mpbdata[id].bteam[3].rolearr[i].rnum].lmagic
            [mpbdata[id].bteam[3].rolearr[i].mag],
            Rrole[mpbdata[id].bteam[3].rolearr[i].rnum].maglevel
            [mpbdata[id].bteam[3].rolearr[i].mag] div 100);
          if Rrole[mpbdata[id].bteam[2].rolearr[i1].rnum].CurrentHP <= 0 then
          begin
            ...
            begin
              ...
              str := str + '在' + gbktounicode(@Rscene[mpbdata[id].snum].Name);
              str := str + '戰勝' +
                gbktounicode(@Rmenpai[mpbdata[id].attmp].Name);
              addtips(str);
              exit;
            end;
          end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if tmp = -1 then
        begin
          HurtValueAI(mpbdata[id].bteam[2].rolearr[i].rnum,
            mpbdata[id].bteam[3].rolearr[i1].rnum,
            Rrole[mpbdata[id].bteam[2].rolearr[i].rnum].lmagic
            [mpbdata[id].bteam[2].rolearr[i].mag],
            Rrole[mpbdata[id].bteam[2].rolearr[i].rnum].maglevel
            [mpbdata[id].bteam[2].rolearr[i].mag] div 100);
          if Rrole[mpbdata[id].bteam[3].rolearr[i1].rnum].CurrentHP <= 0 then
          begin
            ...
            begin
              ...
              begin
                ...
                begin
                  Rrole[mpbdata[id].bteam[3].rolearr[i2].rnum].weizhi :=
                    Rmenpai[Rrole[mpbdata[id].bteam[3].rolearr[i2].rnum]
                    .MenPai].zongduo;
                  Rrole[mpbdata[id].bteam[3].rolearr[i2].rnum].lsweizhi := -1;
                end;
              ...
              str := str + '手中奪取' +
                gbktounicode(@Rscene[mpbdata[id].snum].Name);
              addtips(str);
              exit;
            end;
          end;
...
begin
  ...
  begin
    if mag1 >= 0 then
    begin
      HurtValueAI(Rnum1, Rnum2, Rrole[Rnum1].lmagic[mag1],
        Rrole[Rnum1].maglevel[mag1] div 100);
      if Rrole[Rnum2].CurrentHP <= 0 then
      begin
        Result := True;
        exit;
      end;
      HurtValueAI(Rnum2, Rnum1, Rrole[Rnum2].lmagic[mag2],
        Rrole[Rnum2].maglevel[mag2] div 100);
...
procedure HurtValueAI(Rnum1, Rnum2, mnum, level: integer);
var
  i, l1, c, mhurt, p, def, mp1, mp2, AddAtt, att, spd2, wpn2, spd1, wpn1, k1,
    k2, knowledge, dis, res, tmp: integer;
  a1, s1, m1, w1: double;
begin

  mhurt := CalNewHurtValue(level, Rmagic[mnum].MinHurt, Rmagic[mnum].MaxHurt,
    Rmagic[mnum].HurtModulus);

  p := Rmagic[mnum].AttackModulus * 6 + Rmagic[mnum].MPModulus + Rmagic[mnum]
    .SpeedModulus * 2 + Rmagic[mnum].WeaponModulus * 2;
  ...
  if CheckEquipSet(Rrole[Rnum1].equip[0], Rrole[Rnum1].equip[1],
    Rrole[Rnum1].equip[2], Rrole[Rnum1].equip[3]) = 5 then
  begin
    Inc(att, 50);
    Inc(spd1, 30);
  end;
  if CheckEquipSet(Rrole[Rnum2].equip[0], Rrole[Rnum2].equip[1],
    Rrole[Rnum2].equip[2], Rrole[Rnum2].equip[3]) = 5 then
  begin
    Inc(def, -25);
    Inc(spd2, 30);
  end;
  ...
  Rrole[Rnum1].CurrentMP := max(0, Rrole[Rnum1].CurrentMP - level *
    Rmagic[mnum].NeedMP);
  Rrole_a[Rnum1].CurrentMP := Rrole_a[Rnum1].CurrentMP + Rrole[Rnum1]
    .CurrentMP - tmp;
  tmp := Rrole[Rnum2].CurrentHP;
  Rrole[Rnum2].CurrentHP := max(0, Rrole[Rnum2].CurrentHP - res);
  Rrole_a[Rnum2].CurrentHP := Rrole_a[Rnum2].CurrentHP + Rrole[Rnum2]
    .CurrentHP - tmp;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if mpnum <> k then
      begin
        if random(20000) < (Rmenpai[mpnum].shengwang + Rmenpai[mpnum].dzq * 10 -
          800 - Rmenpai[mpnum].dizi * 20 - random(Rmenpai[mpnum].dizi * 30))
        then
        begin
...
begin
  ...
  for i := 0 to length(Rrole) - 1 do
  begin
    if (Rrole[i].headnum < 0) or
      ((Rrole[i].dtime >= 1000) and ((Rrole[i].nweizhi = 16) or
    ...
    Rrole[i].CurrentHP :=
      max(1, min(Rrole[i].CurrentHP + Rrole[i].MaxHP div 20 +
      (Rrole[i].MaxHP - Rrole[i].CurrentHP) div 20 - (Rrole[i].Hurt +
      Rrole[i].Poision) * Rrole[i].CurrentHP div 200, Rrole[i].MaxHP));
    Rrole_a[i].CurrentHP := Rrole_a[i].CurrentHP + Rrole[i].CurrentHP - tmp;
    tmp := Rrole[i].CurrentMP;
    Rrole[i].CurrentMP :=
      max(1, min(Rrole[i].CurrentMP + Rrole[i].MaxMP div 20 +
      (Rrole[i].MaxMP - Rrole[i].CurrentMP) div 20 - (Rrole[i].Hurt +
      Rrole[i].Poision) * Rrole[i].CurrentMP div 200, Rrole[i].MaxMP));
    Rrole_a[i].CurrentMP := Rrole_a[i].CurrentMP + Rrole[i].CurrentMP - tmp;

    tmp := Rrole[i].PhyPower;
    Rrole[i].PhyPower :=
      max(1, min(100, Rrole[i].PhyPower + 3 - (Rrole[i].Hurt +
      Rrole[i].Poision) div 50));
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        tp := @talkarray[0];
        if (i < 601) then
          allBTalk.talk[(i - 1) div 300][((i - 1) mod 300) div 100]
            [(((i - 1) mod 100) div 10)][((i - 1) mod 100) mod 10] :=
            gbktounicode(tp)
        else if i >= 601 then
          allBTalk.etalk[(i - 601) div 10][(i - 601) mod 10] :=
            gbktounicode(tp);
      end;
    end;
    FileClose(idx);
...
var
  ...
  filename, filename1: AnsiString;
  idx, grp, i1, i2, len, len1, wei, Count: integer;
  RoleOffset: integer;
  tmp: uint16;
  savedata: Tsave;
  ischeat: boolean;
  note: array of AnsiString;
  f: textfile;
  tmpe:uint16;
begin
  Count := 0;
  ...
  begin
    if ((Ritemlist[i1].number - Ritemlist_s[i1].number) <> Ritemlist_a[i1]
      .number) then
    begin
      Inc(Count);
    ...
    if ((Ritemlist[i1].amount - Ritemlist_s[i1].amount) <> Ritemlist_a[i1]
      .amount) then
    begin
      Inc(Count);
      setlength(note, Count);
  ...
  for i1 := 0 to length(Rrole) - 1 do
  begin
    if ((Rrole[i1].PracticeBook - Rrole_s[i1].PracticeBook) <> Rrole_a[i1]
      .PracticeBook) then
    begin
      Inc(Count);
      setlength(note, Count);
    ...
    if ((Rrole[i1].CurrentMP - Rrole_s[i1].CurrentMP) <> Rrole_a[i1].CurrentMP)
    then
    begin
      Inc(Count);
      setlength(note, Count);
    ...
    if ((Rrole[i1].CurrentHP - Rrole_s[i1].CurrentHP) <> Rrole_a[i1].CurrentHP)
    then
    begin
      Inc(Count);
      setlength(note, Count);
    ...
    for i2 := 0 to 29 do
    begin
      if ((Rrole[i1].lmagic[i2] - Rrole_s[i1].lmagic[i2]) <> Rrole_a[i1]
        .lmagic[i2]) then
      begin
        Inc(Count);
        setlength(note, Count);
    ...
    for i2 := 0 to 29 do
    begin
      if ((Rrole[i1].maglevel[i2] - Rrole_s[i1].maglevel[i2]) <> Rrole_a[i1]
        .maglevel[i2]) then
      begin
        Inc(Count);
        setlength(note, Count);
    ...
    for i2 := 0 to 29 do
    begin
      if ((Rrole[i1].lzhaoshi[i2] - Rrole_s[i1].lzhaoshi[i2]) <> Rrole_a[i1]
        .lzhaoshi[i2]) then
      begin
        Inc(Count);
        setlength(note, Count);
    ...
    if ((Rrole[i1].Aptitude - Rrole_s[i1].Aptitude) <> Rrole_a[i1].Aptitude)
    then
    begin
      Inc(Count);
      setlength(note, Count);
    ...
    if ((Rrole[i1].xiangxing - Rrole_s[i1].xiangxing) <> Rrole_a[i1].xiangxing)
    then
    begin
      Inc(Count);
      setlength(note, Count);
    ...
    if ((Rrole[i1].HidWeapon - Rrole_s[i1].HidWeapon) <> Rrole_a[i1].HidWeapon)
    then
    begin
      Inc(Count);
      setlength(note, Count);
    ...
    tmpe := Rrole[i1].exp - Rrole_s[i1].exp;
    if (tmpe <> Rrole_a[i1].exp) then
    begin
      Inc(Count);
      setlength(note, Count);
    ...
    if ((Rrole[i1].PhyPower - Rrole_s[i1].PhyPower) <> Rrole_a[i1].PhyPower)
    then
    begin
      Inc(Count);
      setlength(note, Count);
    ...
    for i2 := 0 to 4 do
    begin
      if ((Rrole[i1].equip[i2] - Rrole_s[i1].equip[i2]) <> Rrole_a[i1].equip[i2])
      then
      begin
        Inc(Count);
        setlength(note, Count);
    ...
    if ((Rrole[i1].knowledge - Rrole_s[i1].knowledge) <> Rrole_a[i1].knowledge)
    then
    begin
      Inc(Count);
      setlength(note, Count);
    ...
    if ((Rrole[i1].AttTwice - Rrole_s[i1].AttTwice) <> Rrole_a[i1].AttTwice)
    then
    begin
      Inc(Count);
      setlength(note, Count);
    ...
    if ((Rrole[i1].jiaoqing - Rrole_s[i1].jiaoqing) <> Rrole_a[i1].jiaoqing)
    then
    begin
      Inc(Count);
      setlength(note, Count);
...
begin
  ...
  begin
    if Rmenpai[mpnum1].guanxi[mpnum2] >= 800 then
    begin
      Result := True;
    end
    else if (Rmenpai[mpnum1].tongmeng >= 0) then
    begin
      if Rmenpai[mpnum1].tongmeng = Rmenpai[mpnum2].tongmeng then
      begin
        Result := True;
      end;
    end;
