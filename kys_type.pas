...
type
  ...
  pyword = record
    hanzi: array[0..3] of AnsiChar;
    pinyin: array[0..8] of AnsiChar;
  end;
  ...
  TRole = record
    case TCallType of
      Element: (ListNum, HeadNum, IncLife, fuyuan: Smallint;
        ...
        zhanwei1:smallint;
        Nick: array[0..9] of AnsiChar;
        zhanwei2:smallint;
        Sexual, Level: Smallint;
        Exp: uint16; //18
        CurrentHP, MaxHP, Hurt, Poision, PhyPower: Smallint; //23
        ExpForItem: uint16;
        Equip: array[0..4] of Smallint;
        Gongti: smallint;
        TeamState: smallint; //31
        Angry: smallint;
        israndomed: smallint;
        ...
        ExpForBook: uint16; //64
        xiangxing, jiaoqing, Rtype, swq, pdq, xxq, jqq, MenPai, shifu, scsx, bssx: smallint; //75
        Choushi: array[0..1] of smallint;
        weizhi, nweizhi, nfangxiang, unuse0, lwq, msq, ldq, qtq, lsweizhi, lsnweizhi, lsfangxiang: smallint;
        Sx, Sy, songshu, gongxian, unuse5, unuse6, unuse7, unuse8, unuse9,btnum: smallint; //98
        LMagic, MagLevel: array[0..29] of smallint; //158
        TakingItem, TakingItemAmount: array[0..3] of smallint;
        jhMagic: array[0..9] of smallint; //176
        LZhaoshi: array[0..29] of smallint;
        MRevent: smallint;
        AllEvent: array[0..8] of smallint;
        leaveevent: smallint);
      Address: (Data: array[0..217] of Smallint);
  end;
  ...
  TItem = record
    case TCallType of
      Element: (ListNum: Smallint;
        ...
        Introduction: array[0..29] of AnsiChar; //36
        zhanwei2:smallint;
        Magic, AmiNum, User, EquipType, ShowIntro, ItemType, inventory, price, EventNum: Smallint; //46
        AddCurrentHP, AddMaxHP, AddPoi, AddPhyPower, ChangeMPType, AddCurrentMP, AddMaxMP: Smallint;
        AddAttack, AddSpeed, AddDefence, AddMedcine, AddUsePoi, AddMedPoi, AddDefPoi: Smallint; //60
        AddFist, AddSword, AddKnife, AddUnusual, AddHidWeapon, AddKnowledge, AddEthics,
        AddAttTwice, AddAttPoi: Smallint; //69
        OnlyPracRole, NeedMPType, NeedMP, NeedAttack, NeedSpeed, NeedUsePoi, NeedMedcine, NeedMedPoi: Smallint; //77
        NeedFist, NeedSword, NeedKnife, NeedUnusual, NeedHidWeapon, NeedAptitude: Smallint; //83
        NeedExp, Count, Rate: Smallint; //86
        NeedItem, NeedMatAmount: array[0..4] of Smallint);
      Address: (Data: array[0..96] of Smallint);
  end;
  ...
  TScene = record
    case TCallType of
      ...
      Address: (Data: array[0..85] of Smallint);
  end;
  ...
  TMagic = record
    case TCallType of
      ...
      Address: (Data: array[0..138] of Smallint);
  end;
  ...
  Tzhaoshi = record
    case TCallType of
      Element: (daihao, congshu, shunwei: Smallint;
        ...
        shuoming: array[0..45] of AnsiChar; //40
        zhanwei2:smallint;
        texiao: array[0..23] of Ttexiao); //89
  ...
  Tmenpai = record
    case TCallType of
      Element: (num: Smallint;
        ...
        tongmeng,unuse1,unuse2,unuse3,unuse4:smallint);
      Address: (Data: array[0..122] of Smallint);
  end;

  TBattleRole = record
    case TCallType of
      Element: (rnum, Team, Y, X, Face, Dead, Step, Acted: Smallint;
        Pic, ShowNumber, showgongji, showfangyu, szhaoshi, Progress, round, speed: Smallint; //15
        ExpGot, Auto, Show, Wait, frozen, killed, Knowledge, LifeAdd, Zhuanzhu, pozhao, wanfang: Smallint; //24
  ...
  TWarSta = record
    case TcallType of
      Element: (BattleNum: smallint;
        ...
        mate, automate, mate_x, mate_y: array[0..11] of smallint; //56
        enemy, enemy_x, enemy_y: array[0..29] of smallint; //146
        BoutEvent, OperationEvent: smallint;
        GetKongfu: array[0..2] of smallint;
        GetItems: array[0..2] of smallint; //154
        GetMoney: smallint);
  ...
  TBZYRole = record
    rnum, snum, isin, mag ,day: smallint;
  end;
  TBZYdui = record
    duizhang:TBZYRole;
    duiyuan:array of TBZYRole;
  end;
  TTmpZYdui = record
  ...
  TMpBdata = record
    ...
    ZYlen:integer;
    zengyuan :array of TBZYdui;
    zaotui: array of TBZaotui;
  end;
  Tscenediaodu = record
...
var
  ...
  Meun_file : PAnsiChar = 'resource\Menu.Pic';
  ...
  MaxProList: array[45..60] of integer = (200, 200, 200, 200, 200, 200, 200, 200, 200, 200,
  ...
  //where: 0-主地图, 1-场景, 2-战场, 3-开头画面,4結束介面  ,6 開始讀取介面
  SaveNum: smallint;
  //存档号, 未使用
  ...
  PROGRESS_PIC,NowPROGRESS_PIC, angryprogress_pic, angrycollect_pic, angryfull_pic: TBpic;
  ROLL_PIC:array[0..1] of TBpic;
  ROLLSTYLE_PIC,WORD_ZHUANGTAI_PIC,WORD_WUGONG_PIC,WORD_XITONG_PIC,WORD_WUPIN_PIC,WORD_SHUPU_PIC,WORD_DUIYOU_PIC: TBpic;
  WORD_XINGGE_PIC,WORD_XINGGEZY_PIC: TBpic;
  WORD_XIANGXING_PIC:array[0..9] of TBpic;
  WORD_YOUHAO_PIC: array[0..7] of TBpic;
  WORD_AIHAO_PIC: array[0..8] of TBpic;
  WORD_SYSTEM_PIC:array[0..4] of TBpic;
  TITLE1_BEGIN_PIC:TBpic;
  TITLE2_BEGIN_PIC:TBpic;
  TITLE3_BEGIN_PIC:TBpic;
  NEW_KMENU_PIC :TBpic;
  TITLE_BEGIN_PIC:array[0..3] of TBpic;
  NEW_MENU_BACKGRAND_PIC :TBpic;
  NEW_MENU_PIC:array of Tpic;
  RENWU_PIC,WORD_RENWU_PIC:TBpic;
  HUIKUANG3_PIC,HUANGKUANG3_PIC,BIAOTIKUANG3_PIC:TBpic;
  DUIWU_XUANZHEKUANG1_PIC,DUIWU_XUANZHEKUANG2_PIC:TBpic;
  WORD_ZHAOSHI_PIC:array[0..4] of TBpic;

  YOUHAO_PIC: TBpic;
  WORD_YOUHAO2_PIC :array[0..7] of TBpic;
  ROLLSTYLE2_PIC:TBpic;
  ROLL2_PIC:array[0..1] of TBpic;
  ...
  //debug输出
  debugfile : textfile;
  debugname : AnsiString = 'debug.txt';
  debugf:integer;
const
  RMask: uint32 = $FF0000;
  GMask: uint32 = $FF00;
